...
| [persistence.md](persistence.md) | The `"LNNS"` wire format byte by byte, the six Save/Load APIs, a runnable train → save → load → resume example, and the untrusted-stream safety contract (validate before allocating, fixed limits, progressive allocation). |
