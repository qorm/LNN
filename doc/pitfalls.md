...
| # | issue | source | severity | disposition |
|---|---|---|---|---|
| 1 | Fused LTC path: a NaN produced by an operation whose operands are **both** NaNs does not preserve its payload/sign bit across the fused and the former graph path (NaN positions and every finite value are bitwise). The fused CfC step (stage 18) carries the identical acceptance level | stage-16 red team | Low (accepted) | Same acceptance class as the single-source sign-bit corner F9-1 — training-irrelevant, no observable trajectory divergence; the payload depends on compiler instruction selection, not on any replicated rounding structure. Contract stated in the NaN-semantics headers of `nn/ltc_fused.go` and `nn/cfc_fused.go` |
