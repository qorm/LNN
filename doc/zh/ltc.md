...
| 构造函数、校验 | `NewLTC` | 104–173 |
| 参数初始化区间 | `NewLTC` 字面量 | 139–151 |
| `eps`（分母保护）= `1e-8` | `ltcEps` 常量；`eps` 字段初始化 | 12–13；138 |
| 构造期图常量（折叠掩码、单位阵、+0 种子、反转电位行视图、突触项表） | struct 注释；`NewLTC` 字面量；`identityMat`；`erevRowViews`；`synapsePlan` | 41–89；155–171；189–202；204–227；229–270 |
| 可训练参数集（13 个张量） | `Parameters()` | 282–289 |
| 一次 RNN 步进 | `Step` | 299–337 |
| `ts` 契约检查（正且有限） | `Step` 防护 | 304–306 |
| 仿射输入映射 `inputs = x⊙inW + inB` | `Step` | 313 |
| 对 `cm, w, sW`（加 `gleak`）的 softplus 约束；掩码每 Step 一次折叠进权重 | `Step` | 318–321 |
| 感知驱动、`numConst`/`denBase` 装配、循环电流与 ODE 展开循环（每个子步的 `num`/`den` 更新） | **单个融合节点**：`fusedUnfolds`（`nn/ltc_fused.go`；前向 + 手写 VJP，逐位一致） | 333（调用） |
| 仿射输出映射 `out = v⊙outW + outB` | `Step` | 335 |
| `cm_t = softplus(cm)·unfolds/ts`，带溢出安全缩放 | `scaledCapacitance` | 353–366 |
| 逐突触前神经元激活块 + 稀疏收缩——**oracle**：逐字保留的融合前图路径，服务于差分门禁 | `synapses` / `synapsesRows` / `contract` | 388–401 / 406–420 / 484–503 |

### 稀疏收缩

`synapses`/`synapsesRows`（`nn/ltc.go:388-420`）以每个突触前神经元一个向量块计算电流，不再是逐突触对的循环。自阶段 16 起，这些方法是 **oracle** 而非热路径：生产 `Step` 跑融合内核（`nn/ltc_fused.go`），它们构建的图路径逐字保留，作为钉住内核逐位一致的差分门禁的参照（`nn/ltc_fused_diff_test.go` 的 `legacyLTCStep`）。每个参数矩阵的第 `i` 行仍然参数化*从*神经元 `i` *出发*的突触：
...
// contract（nn/ltc.go:484-503）以 +0 种子折叠收缩突触前轴，
...
相对最初的逐突触循环，有两处结构性变化源自阶段 6 并保留至今：**掩码移出热路径**——接线掩码以每 Step 一次矩阵 Hadamard 折叠进正值约束后的权重，`wm = softplus(w)⊙mask`（`nn/ltc.go:320-321`），而不是每个子步每个突触一次带掩码的乘法；**逐 Step 切行彻底消失**——阶段 6 那个为展开循环把 Step 不变的循环参数行切一次的 `rows` 助手已不复存在，因为融合内核每 Step 在其暂存里冻结参数矩阵副本（`mu`/`sigma`/`sMu`/`sSigma`），`SliceRow` 节点整体消亡。每 Step 的节点账把这条演进说得最清楚：融合前 **626** 个图节点（`inDim=4, units=16, unfolds=4`），阶段 16 融合展开内核后 **84**，阶段 19a 内化感知路径后**任意维度 34**（15 算子 + 19 叶；`TestLTCFusedNodeAccount19a` 钉住）。
...
实施方差分门禁（1,164 组 × `Float32bits`；oracle = 从 git 提取的指示阵发布版实现：LTC/CfC × 全连与稀疏接线 × 多种规模/时间步/状态，含 12 组反向跳零对抗角）**零差异**；红队独立复测（522 组刻意异源分布：前向 + 全 13 参数 + 叶梯度逐位相同）相互印证。CfC 的 contract 与 LTC 逐行同构（`nn/cfc.go:321-337`）。
...
**实测开销。** 当前值（同机，`-benchtime=200x`，复验过）：`LTCStep` **77 allocs/op**、`UnrollBackward` **3,750 allocs/op**——较最初逐突触循环（7,360 / 120,163）累计 **−99%/−97%**（阶段 6 向量化降至 3,440 / 68,688，阶段 7 反向深改把反向降至 33,963，阶段 8 Sigmoid–Hadamard 融合再降至 2,306 / 31,983，阶段 10 内嵌形状 backing（embedded backing）降至 2,707 / 31,994——allocs 再降 −18%/−23% 而墙钟持平，阶段 16 融合展开内核降至 236 / 6,662，阶段 19（感知路径入核、VJP 暂存复用、DFS 暂存池化、广播定长数组）降至当前值——见 [architecture.md](architecture.md)）。阶段 9 这一步的诚实披露：稀疏收缩使 allocs 相对阶段 8 **上升**约 43%/30%（折叠每级的 opAdd b 支克隆）——工单预期"中性或略降"错了——但 **ns/op 下降约 21%/13%**（红队独立复测），因消灭了稠密指示阵 MatMul 对零行的 O(units³) 空转内层循环与反向大分配。**分配次数换走了无用算力，墙钟净受益**。example 的首轮 loss 仍是 `0.690761`，与重写前逐位一致。
...
代码逐元素计算 `v ← num / (den + eps)`（在融合内核内部，`nn/ltc_fused.go`），其中 `cm/dt = softplus(cm)·unfolds/ts` 由 `scaledCapacitance` 构建。所有量都是按单元的向量，因此每个运算都是逐元素或广播；退化情形（所有接线掩码为零）精确约化为一个漏电积分器 `v ← (a·v + b·vleak)/(a + b + eps)`，其中 `a = softplus(cm)·unfolds/ts`、`b = softplus(gleak)`——由 `nn/ltc_test.go` 中的 `TestLTCZeroMasksLeakyIntegrator` 以闭式做了回归测试。
...
`Parameters()` 返回这 13 个可训练张量；`erev`/`sErev` 被刻意排除在外（`nn/ltc.go:282-289`）。学习反转电位会让突触在兴奋性与抑制性极性之间翻转，把 LTC 退化为普通可塑网络——±1 的符号模式是结构性的，在构造时抽取一次。
...
**契约：`ts` 必须为正且有限。** `NaN`、`+Inf`、`-Inf`、零和负值都会 panic（`nn/ltc.go:304-306`）：
...
实现的有限性域（`scaledCapacitance`，`nn/ltc.go:353-366`）：
...
| ODE 求解器：`unfolds` 个子步上的半隐式欧拉 | 相同方案（已融合为单个图节点，`nn/ltc_fused.go`） |
