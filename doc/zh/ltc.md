...
**实测开销。** 当前值（同机，`-benchtime=200x`，复验过）：`LTCStep` **3,296 allocs/op**、`UnrollBackward` **41,588 allocs/op**——较最初逐突触循环（7,360 / 120,163）累计 **−55%/−65%**（阶段 6 向量化降至 3,440 / 68,688，阶段 7 反向深改把反向降至 33,963，阶段 8 Sigmoid–Hadamard 融合再降至 2,306 / 31,983——见 [architecture.md](architecture.md)）。阶段 9 这一步的诚实披露：稀疏收缩使 allocs 相对阶段 8 **上升**约 43%/30%（折叠每级的 opAdd b 支克隆）——工单预期"中性或略降"错了——但 **ns/op 下降约 21%/13%**（红队独立复测），因消灭了稠密指示阵 MatMul 对零行的 O(units³) 空转内层循环与反向大分配。**分配次数换走了无用算力，墙钟净受益**。example 的首轮 loss 仍是 `0.690761`，与重写前逐位一致。
