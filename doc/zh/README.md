...
| [ltc.md](ltc.md) | 液态时间常数细胞，逐式对照代码：ODE、半隐式欧拉（semi-implicit Euler）代数推导、向量化突触驱动、参数表、`ts` 契约与接线。 |
...
| [architecture.md](architecture.md) | 三层设计（tensor → autograd → nn）外加 optimizer 包、张量为何没有 stride、计算图及其反向闭包如何工作。 |
| [pitfalls.md](pitfalls.md) | 红队审计得出的已知边界与残余风险，以用户须知形式呈现：并发、float32 溢出、重复 Backward、微小 `ts`，以及技术债路线图。 |
...
5. **[architecture.md](architecture.md)** —— 调试与扩展所需的心智模型。
6. **[pitfalls.md](pitfalls.md)** —— 上线之前必读。
