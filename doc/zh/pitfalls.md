...
| AdEMAMix + Schedule-Free AdamW 优化器；`"LNO1"` kind 3/4 | **已完成（阶段 20）：** 两条新更新规则，各自对照其论文的第三方 float64 参考移植（AdEMAMix，arXiv:2409.03137——双梯度 EMA，慢速者刻意不校正，α/β3 warmup 调度；ScheduleFreeAdamW，arXiv:2405.15682——以迭代平均取代衰减调度，附 `Train`/`Eval` y↔x 转换契约与逐参数模式位：对 eval 模式参数 `Step` 先于任何修改 panic 指名）。对参考最大偏差 9.5e-6 / 3.3e-7，带鉴别力守门；两算法 50+50 vs 100 续训逐位（含横跨 warmup 边界）。`"LNO1"` 新增 kind 3/4（AdEMAMix 沿用 Adam 记录布局；ScheduleFree 增加 `k`/`pow2`/`lrMax`/`wsum`/`mode`——mode 字节使 eval 检查点可跨实例续训；`lrMax`/`wsum` 非有限按损坏拒载），version 保持 1——扩展标签而非迁移；kind 0–2 字节零改动（手写旧 kind2 流照收）。指南：[training.md](training.md)（方程与选用指引）、[persistence.md](persistence.md)（线格式）、[cookbook.md](cookbook.md) 食谱 14。残余（信息级）：优化器级模式标志只对无状态参数把门（安全侧过杀，与上游一致）；参数在 `Step` 中途 resize 的 panic 非原子（既往存在，全部优化器同类） |
