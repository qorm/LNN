...
阶段 19 两次外推了这条边界。**19a 把感知驱动内化**，连同 `numConst`/`denBase` 装配（每步再内化约 `10·inDim+2` 个节点，残余解释开销中最大的一块）：父列表 9 → 12（`inputs`、`vleak`、`sMu`、`sSigma`、`sWm` 入核），`hvN` 投递槽位是*重新推导*而非沿用——过去在 `numConst` 子树展开期间追加的一切，现在改在 `inputs` 父节点的子树展开期间追加，同在 `hvN` 之后的位置，三条已文档化的交错关系机械保持——一个 Step 现在**任意维度下记录 34 个节点**（15 算子 + 19 叶；此前 84，融合前 626；`TestLTCFusedNodeAccount19a` 钉住）。**19b 让 VJP 的暂存面每次反向只分配一次**，跨展开轮与突触前行复用（18 块平面），交付/复用分界写明：复用暂存绝不交给 `addGrad`，而每一块交付的梯度缓冲仍是新分配的——`addGrad` 首次贡献的所有权移交是逐位契约的一部分（最末展开的累加器行为拷贝领养，−0 符号含）。remat 折叠类不变（`TestRematFusedLTCFoldClasses` 钉住）。本机实测（`-benchtime=200x`）：`LTCStep` 约 87 µs / 236 → **约 78 µs / 77 allocs / 23 KB**，`UnrollBackward` 约 1.33 ms / 6,662 → **约 1.29 ms / 3,750 / 558 KB**，`UnrollRemat` 约 3.45 ms / 16,000 → **约 3.4 ms / 9,373**，`UnrollRematCfC` 约 0.83 ms / 5,000 → **约 0.84 ms / 4,829**（CfC 本轮未触——数字沿用阶段 18）。
...
内存语义：峰值图内存降为 O(chunkSize × 逐步子图) 加 O(T) 个 detach 小张量（每步一个 `[batch, StateSize()]`）——T = 512 实测：**保留约 0.65 MB 对比全展开驻留约 8.3 MB**（约 13×）。最坏情形如实相告：以对抗性次序访问输出的损失（降序访问配小 chunkSize 是极端）迫使重算单元合并到 O(T) 长，峰值内存回到全展开的数字——还要叠加各扫描的重算代价。在这个角落，`UnrollRemat` 的峰值内存*和*算力都严格贵于一次全图反向；这是逐位保真的代价，不是 bug。调用方契约全部由探针以 panic 强制：`params` 必须列出细胞 `Step` 消费的**每一个**可训练叶（漏列的叶会跨扫描静默累加出 2–3 倍的错梯度）；细胞逐步图结构必须是 `(x, h)` 的纯函数——值依赖的分支是探针唯一看不见的情形，会漂约 1–2 ULP，已留档 [pitfalls.md](pitfalls.md)；闭合引用了步侧消费叶的损失，必须把该叶的每个 loss 侧消费者安排在被播种输出之后访问（惩罚项请写成数据在前：`Add(data, penalty)`）。分类缓存曾被考虑并刻意否决——不存在健全的缓存键，陈旧命中会静默跳过多类 panic（两处 loss 图遍历则共享一次 `TopoOrder` 计算）。含 chunk 选型指引的可运行食谱见 [cookbook.md](cookbook.md#13-长序列训练unrollremat-分块-bptt)。
