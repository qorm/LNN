...
The `LTC` type doc comment (`nn/ltc.go:12-26`) states the same model in the
...
|---|---|---|
| ODE statement + integration scheme (doc) | `LTC` type comment | 12–26 |
| Constructor, validation | `NewLTC` | 54–71 |
| Parameter init ranges | `NewLTC` literal | 87–105 |
| `eps` (denominator guard) = `1e-8` | `eps` field init | 88 |
| Trainable parameter set (13 tensors) | `Parameters()` | 126–133 |
| One RNN step | `Step` | 139–178 |
| `ts` contract check (positive & finite) | `Step` guard | 144–146 |
| Affine input map `inputs = x⊙inW + inB` | `Step` | 153 |
| Softplus constraints on `cm, w, sW` (+`gleak`) | `Step` | 156–159 |
| Sensory synaptic currents (once per step) | `synapses(inputs, sMu, sSigma, sW, sErev, …)` | 162 |
| ODE unfold loop (`unfolds` substeps) | `for t := 0; t < c.unfolds; t++` | 166–174 |
| Recurrent currents (recomputed per substep) | `synapses(v, mu, sigma, w, erev, …)` | 167 |
...
| Affine output map `out = v⊙outW + outB` | `Step` | 176 |
| `cm_t = softplus(cm)·unfolds/ts` with overflow-safe scaling | `scaledCapacitance` | 194–207 |
| Synapse activation `w·σ(σp·(vpre−μ))·mask` and current accumulation | `synapses` | 215–238 |

Inside `synapses`, per presynaptic neuron `i` (row `i` of each matrix
parameterizes the synapses *from* `i`):

```go
act := Sigmoid(Hadamard(sigR, Sub(preCol, muR))) // σᵢⱼ·(v_pre,i − μᵢⱼ), then sigmoid
act  = Hadamard(act, wR)                         // × wᵢⱼ
act  = Hadamard(act, Const(maskRow(i)))          // × wiring mask
rev  = Hadamard(act, erevR)                      // actᵢⱼ · erevᵢⱼ
num += rev;  den += act                          // accumulate currents
```
...
(`nn/ltc.go:169-173`), with `cm/dt = softplus(cm)·unfolds/ts` built by
...
deliberately absent (`nn/ltc.go:126-133`). Learning the reversal potentials
...
zero and negative values panic (`nn/ltc.go:144-146`):
...
| ODE solver: semi-implicit Euler over `unfolds` substeps | identical scheme (`Step` loop, `nn/ltc.go:166-174`) |
...
| CfC (closed-form continuous-time) cell | **roadmap — not implemented** |
