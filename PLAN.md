...
# 阶段 21 规划（2026-08-03 追加）

> 依据：阶段 20 关闭（v0.6.0）时留档的阶段 15 外部调研全部遗留项（LNNS CRC32C 格式 v2 独立工单 + PGO 画像重采文档项）；库主令「整体分析 + 规划 + 多 agent 编排 + 红队审计 + 进度同步」，范围拍板**两线全做**。
> 基线：HEAD `39c3911`（v0.6.0 文档态），主控复验 build/vet/gofmt 全绿、工作树干净、五包覆盖率 100/100/100/100/99.6、11 fuzz 门禁 + 双架构 CI。

## 两线工作项

| 线 | 内容 | 依据 | 验收 |
|---|---|---|---|
| ① CRC32C 完整性（LNNS 格式 v2） | serialize 增加完整性校验和：format v1→v2（writer 一律写 v2，reader 兼容 v1+v2）；校验和可增量计算（不破坏未知长度读端渐进分配纪律）；先校验后分配（恶意/损坏流早期带值拒绝）；v1 黄金向量保留为 v1 读路径 fixture，新增 v2 黄金向量冻结 v2 布局 | 阶段 15 调研：本库安全/可审计/确定性三维已严于 safetensors/GGUF/PyTorch，**唯一缺口 CRC32C 完整性**（safetensors 官方 2023 起欠账同类）；「格式版本化」既有规则为 v2 升级定义了契约 | 五包 -race 全绿；serialize 覆盖率向 100% 看齐；篡改对抗（翻转 magic/version/形状/数据/校验和段各一 → 带值 error + 零副作用）；v1 流仍逐位可读；v2 round-trip 逐位（含 NaN/−0）；模型级（nn Save/Load）继承校验；API 零破坏（新增导出需论证）；读端一致性（已知/未知长度 outcome 同） |
| ② PGO 画像重采 | 以 git archive HEAD（v0.6.0 终态）快照在 /tmp 重采 PGO 画像，复测五基准（无 PGO vs 新画像 PGO vs 旧画像 PGO），裁决既有「双态性」结论在新树的表现，刷新 doc/pgo.md 双语数字与断言 | 阶段 19 实测陈旧画像下 LTCStep 趋势 +13.5% 偏劣并建议重采（pgo.md 已自我披露画像陈旧）；PGO 是阶段 15 唯一纯赚项 | 报告含测量命令/三份数字表/双态性裁决/陈旧断言修正清单；pgo.md 双语同构、数字与实测一致；零库 .go 改动（Makefile 可微调并论证） |

## 执行编排

```
21a（并行）┬─ Agent-A：CRC32C 格式 v2（serialize + nn/save.go + 测试）
           └─ Agent-D：PGO 画像重采 + pgo.md 双语刷新（/tmp HEAD 快照测量，不碰库 .go）
21b ─ 红队审计：CRC32C 格式兼容/篡改绕过/校验段布局 + PGO 实测数字复核；主控 gauntlet
21c ─ 文档同步：persistence.md 双语 LNNS v2 规格/安全契约 + api.md（如有新符号）+ pitfalls/README 交叉引用；PROGRESS.md 阶段 21 落账
21d ─ 主控终验 → 呈库主裁决提交/tag（建议 v0.6.1：无新 API 或仅内部演进）或 v0.7.0（如有 API 变更）
```

## 阶段验收（DoD）

- 既有门禁全绿：build/vet/gofmt 空/五包 `-race`/fuzz-smoke 11 目标
- 新代码覆盖率向库标准看齐（正当 100% 或诚实留档）
- 线①篡改对抗与 v1/v2 双布局测试常驻化；线②数字与实测一致、诚实留痕
- 基准数字主控独立复验；文档断言回源核对
- PROGRESS.md 逐阶段同步；git 提交/打 tag 前逐项请示库主

---

# 阶段 22 规划（2026-08-03 追加）

> 依据：库主令「ops.go 拆分、包注释清理」——遗留登记表 #6（ops.go 拆分 🟢 低优先）+ #8（包注释清理）。
> 勘察结论（主控）：**#8 已在前几阶段顺带清零**——五包各恰一处包注释真源（tensor/autograd/nn/optimizer 经 doc.go，serialize 经 serialize.go），源码内零陈旧重复，本项转审计收尾；可执行核心为 **ops.go 纯文件拆分**。
> 基线：HEAD `55cdba9`（v0.6.1 发布态），工作树干净。

## 工作项

| # | 内容 | 依据 | 验收 |
|---|---|---|---|
| ① | `tensor/ops.go`（631 行 34 函数）按职责拆分为独立文件（matmul/broadcast/elementwise/slice/reduce 等，方案由实施自拟并论证） | 遗留 #6；核心分析 A「五职责挤一文件」 | 纯文件移动零内容变更（每被移函数体逐字节一致）；build/vet/gofmt/五包 race 全绿；覆盖率与 v0.6.1 逐包持平；API 面零变更 |
| ② | `autograd/ops.go`（1000 行）拆为引擎（opKind/runBackward/私有梯度助手）与公共算子构造器两文件 | 同上 | 同 ① 门禁 |
| ③ | 五包注释单一真源审计（doc.go 覆盖准确性核对；预期零改动或极小修正） | 遗留 #8 已清零 | 每包恰一处 `// Package`；grep 无陈旧重复；doc.go 表述与现 API 核对 |

## 执行编排

```
22a ─ 实施 Agent：①+② 纯文件拆分 + ③ 审计（单 agent，两文件拆分无并行冲突）
22b ─ 红队独立验证（拆分纯移动性 / API 面 / 覆盖率）+ 主控 gauntlet
22c ─ PROGRESS.md 阶段 22 落账 → 呈库主提交裁决（建议直接提交，不打 tag——纯维护零 API 变化）
```

## DoD

- gofmt 空 / build / vet / 五包 `-race` 全绿；覆盖率逐包与 v0.6.1 持平
- 拆分 = 纯移动：`git diff` 对每个被移函数体逐字节无差异
- API 面零变更（导出符号集合与 v0.6.1 一致，`go doc` 核对）
- 包注释审计：五包各恰一处真源、doc.go 表述准确
- PROGRESS.md 落账；提交前请示库主
